...
TEST_F(LSTest, Sixtyfour_Intel_ls) {
  ...
  EXPECT_STREQ(m_parser.getSectionHeaders().getSections()[3].getName().c_str(),
               ".note.gnu.build-id");
  ...
  EXPECT_STREQ(m_parser.getProgramHeaders().getProgramHeaders()[6].getName().c_str(),
               "GNU_EH_FRAME");
...
TEST_F(LSTest, Sixtyfour_Intel_ls_upx)
{
    ...
    for(std::vector<std::pair<boost::int32_t, std::string> >::const_iterator reasons =
m_parser.m_reasons.begin(); reasons != m_parser.m_reasons.end(); ++reasons)
    {

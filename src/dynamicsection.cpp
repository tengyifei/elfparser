...
boost::uint64_t DynamicSection::getOffset() const { return m_offset; }
...
void DynamicSection::createDynamic(const char* p_start, boost::uint32_t p_offset,
  ...
  if (p_is64) {
    doDynamic64(reinterpret_cast<const elf::dynamic::dynamic_64*>(p_start + p_offset), p_start,
                p_start + p_offset + p_size, p_baseAddress, p_isLE);
  } else {
    doDynamic32(reinterpret_cast<const elf::dynamic::dynamic_32*>(p_start + p_offset), p_start,
                p_start + p_offset + p_size, p_baseAddress, p_isLE);
  }
    ...
    switch (entry.getTag()) {
      ...
      case elf::dynamic::k_gnuhash:
        if (m_symbolTableSize == 0) {
          const boost::uint32_t* hashStart = reinterpret_cast<const boost::uint32_t*>(
              p_start + (entry.getValue() - p_baseAddress));
          ++hashStart;
...
void DynamicSection::doDynamic64(const elf::dynamic::dynamic_64* p_dynamic, const char* p_start,
                                 const char* p_end, boost::uint64_t p_baseAddress, bool p_isLE) {
...
void DynamicSection::doDynamic32(const elf::dynamic::dynamic_32* p_dynamic, const char* p_start,
                                 const char* p_end, boost::uint64_t p_baseAddress, bool p_isLE) {
  for (; reinterpret_cast<const char*>(p_dynamic) < p_end; ++p_dynamic) {
...
boost::uint64_t DynamicSection::getStringTableSize() const { return m_stringTableSize; }

boost::uint32_t DynamicSection::getSymbolTableSize() const { return m_symbolTableSize; }

boost::uint64_t DynamicSection::getInitArray() const { return m_initArrayVirtAddress; }

boost::uint32_t DynamicSection::getInitArrayEntries() const { return m_initArrayEntries; }

void DynamicSection::evaluate(
  ...
  if (!m_entries.empty() && needed.empty()) {
    p_reasons.push_back(
        std::make_pair(10, std::string("Contains dynamic table with no needed shared objects.")));
  }
}

...
StringTableSegment::StringTableSegment(const char* start, boost::uint32_t p_offset,
                                       boost::uint32_t p_size, elf::section_type p_type)
    : SegmentType(start, p_offset, p_size, p_type), m_stringsSet(), m_start(start + p_offset) {
...
std::size_t StringTableSegment::getSize() const { return m_stringsSet.size(); }
...
std::string StringTableSegment::stringLookup(std::size_t p_index) const {
  ...
  size_t length = 0;
  int status = 0;
  unmangled = abi::__cxa_demangle(m_start + p_index, unmangled, &length, &status);
  if (unmangled != NULL) {
...
std::string StringTableSegment::printToStdOut() const {
  ...
  return_value << "String Table (offset=0x" << std::hex << m_offset << ", size=" << std::dec
               << m_size << ", entries=" << m_stringsSet.size() << ")\n";

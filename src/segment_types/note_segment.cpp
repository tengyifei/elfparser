...
const std::map<boost::uint32_t, std::string> type_map = boost::assign::map_list_of(
    1, "NT_GNU_ABI_TAG")(2, "NT_GNU_HWCAP")(3, "NT_GNU_BUILD_ID")(4, "NT_GNU_GOLD_VERSION");

const std::map<boost::uint32_t, std::string> abi_map =
    boost::assign::map_list_of(0, "OS Linux")(1, "OS GNU")(2, "OS Solaris")(3, "OS FreeBSD");
...
NoteSegment::NoteSegment(const char* p_start, boost::uint32_t p_offset, boost::uint32_t p_size,
  ...
  if (m_note->m_nameSize > 1 && m_note->m_nameSize < p_size) {
    m_name.assign(p_start + p_offset + sizeof(elf::note), m_note->m_nameSize - 1);
  }
...
std::string NoteSegment::printToStdOut() const {
  ...
  return_value << "Note Segment (offset=0x" << std::hex << m_offset << ", size=" << std::dec
               << m_size << ")\n";

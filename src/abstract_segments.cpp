...
std::set<std::string> AbstractSegments::getFiles() const {
  ...
  BOOST_FOREACH (const Symbols& symbol, m_otherSymbols) {
    ...
    BOOST_FOREACH (const std::string& value, unique) { combined.insert(value); }
  }
  return combined;
}

void AbstractSegments::setStart(const char* p_data, boost::uint32_t p_size, bool p_is64,
                                bool p_isLE, bool p_isDY) {
  m_data = p_data;
  m_size = p_size;
  m_is64 = p_is64;
  m_isLE = p_isLE;
...
void AbstractSegments::makeSegmentFromSectionHeader(const AbstractSectionHeader& p_header) {
  m_sections.emplace_back(p_header.getName(), p_header.getTypeString(), p_header.getPhysOffset(),
                          p_header.getVirtAddress(), p_header.getSize(), p_header.getLink(),
...
void AbstractSegments::makeSegmentFromProgramHeader(const AbstractProgramHeader& p_header) {
  ...
  m_programs.emplace_back(
      p_header.getName(), p_header.getName(), p_header.getOffset(), p_header.getVirtualAddress(),
      p_header.getMemorySize() ? p_header.getMemorySize() : p_header.getFileSize(), 0,
      p_header.isExecutable(), p_header.isWritable(), p_header.getType() == elf::k_pdynamic);
}

void AbstractSegments::createDynamic() {
  BOOST_FOREACH (const Segment& program, m_programs) {
    if (program.isDynamic()) {
      m_dynamic.createDynamic(m_data, program.getPhysOffset(), program.getSize(), m_baseAddress,
                              m_is64, m_isLE, *this);
      ...
      m_dynSymbols.createSymbols(m_data, m_size, symTab, m_dynamic.getSymbolTableSize(), strTab,
                                 m_dynamic.getStringTableSize(), *this, m_is64, m_isLE, m_isDY);
  ...
  BOOST_FOREACH (const Segment& section, m_sections) {
    if (section.isDynamic()) {
      m_dynamic.createDynamic(m_data, section.getPhysOffset(), section.getSize(), m_baseAddress,
                              m_is64, m_isLE, *this);
...
void AbstractSegments::generateSegments() {
  ...
  for (size_t i = 0; i < all_segments.size(); i++) {
    ...
    if (m_offsets.find(m_data + section.getPhysOffset()) == m_offsets.end()) {
      if (section.getType() == "K_NOTE" || section.getType() == "PT_NOTE") {
        m_types.push_back(
            new NoteSegment(m_data, section.getPhysOffset(), section.getSize(), elf::k_note));
        m_offsets.insert(m_data + section.getPhysOffset());
      } else if (section.getType() == "K_PROGBITS" && section.getName() == ".comment") {
        m_types.push_back(new CommentSegment(m_data, section.getPhysOffset(), section.getSize(),
                                             elf::k_progbits));
        m_offsets.insert(m_data + section.getPhysOffset());
      } else if (section.getType() == "K_PROGBITS" && section.getName() == ".gnu_debuglink") {
        m_types.push_back(new DebugLinkSegment(m_data, section.getPhysOffset(), section.getSize(),
                                               elf::k_progbits));
        m_offsets.insert(m_data + section.getPhysOffset());
      } else if ((section.getType() == "K_PROGBITS" && section.getName() == ".interp") ||
                 (section.getType() == "PT_INTERP")) {
        m_types.push_back(
            new InterpSegment(m_data, section.getPhysOffset(), section.getSize(), elf::k_progbits));
        m_offsets.insert(m_data + section.getPhysOffset());
      } else if (section.getType() == "K_PROGBITS" && section.getName() == ".rodata") {
        m_types.push_back(new ReadOnlySegment(m_data, section.getPhysOffset(), section.getSize(),
                                              elf::k_progbits));
        m_offsets.insert(m_data + section.getPhysOffset());
      } else if (section.getType() == "K_PROGBITS" && m_ctorsArray.getOffset() == 0 &&
                 section.getName() == ".ctors") {
  ...
  BOOST_FOREACH (std::size_t index, symTab) {
    ...
    if (m_sections.size() > m_sections[index].getLink() &&
      ...
      if (m_sections[link].getPhysOffset() != 0 && m_sections[link].getVirtAddress() != 0 &&
          m_sections[link].getVirtAddress() == m_dynamic.getStringTableVirtualAddress() &&
          m_sections[link].getPhysOffset() !=
              getOffsetFromVirt(m_dynamic.getStringTableVirtualAddress())) {
        m_fakeDynamicStringTable = true;
      } else {
        m_types.push_back(new StringTableSegment(m_data, m_sections[link].getPhysOffset(),
                                                 m_sections[link].getSize(), elf::k_strtab));
      ...
      otherSymbols->createSymbols(m_data, m_size, m_sections[index].getPhysOffset(),
                                  m_sections[index].getSize(), m_sections[link].getPhysOffset(),
                                  m_sections[link].getSize(), *this, m_is64, m_isLE, m_isDY);
...
boost::uint64_t AbstractSegments::getBaseAddress() const { return m_baseAddress; }
...
void AbstractSegments::evaluate(
    std::vector<std::pair<boost::int32_t, std::string>>& p_reasons,
  ...
  BOOST_FOREACH (const SegmentType& seg, m_types) { seg.evaluate(p_reasons, p_capabilities); }

  BOOST_FOREACH (const Symbols& sym, m_otherSymbols) { sym.evaluate(p_reasons, p_capabilities); }
...
std::vector<AbstractSymbol> AbstractSegments::getAllSymbols() const {
  ...
  BOOST_FOREACH (const Symbols& other, m_otherSymbols) {
    ...
    BOOST_FOREACH (const AbstractSymbol& newSymbols, others) { symbols.push_back(newSymbols); }
  }

  return symbols;
}

const DynamicSection& AbstractSegments::getDynamicSection() const { return m_dynamic; }

const Symbols& AbstractSegments::getDynamicSymbols() const { return m_dynSymbols; }
...
std::string AbstractSegments::printToStdOut() const {
  ...
  BOOST_FOREACH (const SegmentType& seg, m_types) { returnValue << seg.printToStdOut(); }

  BOOST_FOREACH (const Symbols& sym, m_otherSymbols) { returnValue << sym.printToStdOut(); }

...
AbstractSymbol::AbstractSymbol(const char* p_data, boost::uint32_t p_offset, bool p_is64,
                               bool p_isLE)
    : m_symbol32(), m_symbol64(), m_name(), m_is64(p_is64), m_isLE(p_isLE) {
  if (m_is64) {
...
std::string AbstractSymbol::getTypeName() const { return getSymType(getInfo()); }

std::string AbstractSymbol::getBinding() const { return getSymBinding(getInfo()); }
...
const std::string& AbstractSymbol::getName() const { return m_name; }

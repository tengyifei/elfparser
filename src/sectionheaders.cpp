...
SectionHeaders::SectionHeaders() : m_sectionHeaders(), m_totalSize(0), m_stringIndex(0) {}
...
void SectionHeaders::setHeaders(const char* p_data, const char* p_start,
                                ...
                                boost::uint16_t p_size, std::size_t p_stringIndex, bool p_is64,
                                bool p_isLE,
  ...
  for (std::size_t i = 0; i < p_count; ++i, p_data += p_size) {
    if ((p_start + p_total_size) > p_data) {
      m_sectionHeaders.emplace_back(p_data, p_size, p_start, p_total_size, p_stringIndex,
                                    m_sectionHeaders, p_is64, p_isLE);
      if (m_sectionHeaders.back().getType() != elf::k_nobits &&
          (m_sectionHeaders.back().getPhysOffset() + m_sectionHeaders.back().getSize()) >
              m_totalSize) {
...
void SectionHeaders::evaluate(
  ...
  if (m_sectionHeaders.empty()) {
    p_reasons.push_back(
        std::make_pair(10, std::string("Sections have been stripped or obfuscated")));
    return;
  }
}
...
boost::uint32_t SectionHeaders::getStringTableIndex() const { return m_stringIndex; }

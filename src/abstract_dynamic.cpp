...
std::string createValue(boost::uint64_t p_tag, boost::uint64_t p_value, const char* p_strTab) {
  switch (p_tag) {
    case elf::dynamic::k_needed:
    case elf::dynamic::k_soname:
    case elf::dynamic::k_rpath:
...
AbstractDynamicEntry::AbstractDynamicEntry(boost::uint64_t p_tag, boost::uint64_t p_value)
    : m_tag(p_tag), m_value(p_value), m_stringValue() {}

AbstractDynamicEntry::AbstractDynamicEntry(const AbstractDynamicEntry& p_rhs)
    : m_tag(p_rhs.m_tag), m_value(p_rhs.m_value), m_stringValue(p_rhs.m_stringValue) {}
...
boost::uint64_t AbstractDynamicEntry::getTag() const { return m_tag; }

boost::uint64_t AbstractDynamicEntry::getValue() const { return m_value; }

bool AbstractDynamicEntry::hasString() const { return !m_stringValue.empty(); }

const std::string& AbstractDynamicEntry::getString() const { return m_stringValue; }

void AbstractDynamicEntry::setString(const std::string& p_value) { m_stringValue.assign(p_value); }

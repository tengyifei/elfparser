...
std::map<std::string, std::pair<elf::Capabilties, std::string>> capabilities =
    boost::assign::map_list_of("fclose", std::make_pair(elf::k_fileFunctions, "fclose() found"))(
...
Symbols::Symbols() : m_isDY(false), m_symbols(), m_files() {}

Symbols::~Symbols() {}

const std::vector<AbstractSymbol>& Symbols::getSymbols() const { return m_symbols; }

std::set<std::string> Symbols::getFiles() const { return m_files; }

void Symbols::createSymbols(const char* p_data, boost::uint64_t p_dataSize,
                            boost::uint64_t p_symTabOffset, boost::uint32_t p_symTabSize,
                            boost::uint64_t p_strTabOffset, boost::uint64_t p_strTableSize,
                            const AbstractSegments& p_segments, bool p_is64, bool p_isLE,
                            bool p_isDY) {
  m_isDY = p_isDY;
  ...
  boost::uint8_t multiplier =
      p_is64 ? sizeof(elf::symbol::symtable_entry64) : sizeof(elf::symbol::symtable_entry32);
    ...
    if (symbol.getNameIndex() < p_strTableSize) {
      ...
      if (nullFound) {
        if (symbol.getType() == elf::symbol::k_function) {
          ...
          std::size_t length = 0;
          int status = 0;
          unmangled = abi::__cxa_demangle(p_data + p_strTabOffset + symbol.getNameIndex(),
                                          unmangled, &length, &status);
          if (unmangled != NULL) {
            if (status == 0) {
...
void Symbols::evaluate(std::vector<std::pair<boost::int32_t, std::string>>& p_reasons,
  ...
  BOOST_FOREACH (const AbstractSymbol& entry, m_symbols) {
    if ((entry.getType() & 0x0f) == elf::symbol::k_file) {
      std::map<std::string, std::pair<elf::Capabilties, std::string>>::const_iterator it =
          files.find(entry.getName());
      if (it != files.end()) {
      ...
      std::map<std::string, std::pair<elf::Capabilties, std::string>>::const_iterator it =
          capabilities.find(entry.getName());
  ...
  if (m_symbols.size() > 10 && ((m_symbols.size() - 2) == noType)) {
    ...
    p_capabilities[elf::k_antidebug].insert(
        "Almost all symbols marked as NO_TYPE in they symbol table");
  }
}
...
std::string Symbols::printToStdOut() const {
  ...
  BOOST_FOREACH (const AbstractSymbol& symbol, m_symbols) {
    returnValue << "\t"
                << "type=" << symbol.getTypeName() << ", binding=" << symbol.getBinding()
                << ", value=0x" << std::hex << symbol.getValue() << ", name=" << symbol.getName()
                << "\n";
  }
